  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="p-5 bg-secondary/20 rounded-lg border border-border/30">
        ...
        <div className="mb-4">
          {/* Solo Pago contra entrega por ahora */}
          <div
            className="relative p-4 rounded-lg border-2 border-primary bg-primary/10 shadow-lg"
          >
...
// COD-only form (no Stripe hooks required)
const CODCheckoutForm = ({
  ...
  // Track AddPaymentInfo once on mount
  const initialTrackDoneRef = useRef(false);
  useEffect(() => {
    if (!initialTrackDoneRef.current) {
      trackAddPaymentInfo({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let codOrderId = `COD-${customerData.orderNumber}-${Date.now()}`;
      if (isPriorityShipping) codOrderId += '-PRIORITY';
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="p-5 bg-secondary/20 rounded-lg border border-border/30">
        ...
        <div className="relative p-4 rounded-lg border-2 border-primary bg-primary/10 shadow-lg">
          <div className="flex flex-col items-center gap-2 text-center">
            <BanknotesIcon className="w-6 h-6 text-primary" />
            <div>
              <p className="text-sm font-semibold text-primary">Pagar al Recibir</p>
              <p className="text-xs text-muted-foreground mt-1">Efectivo / QR / Transferencia</p>
            </div>
          </div>
          <div className="absolute top-2 right-2 w-5 h-5 bg-primary rounded-full flex items-center justify-center">
            <svg className="w-3 h-3 text-foreground" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div className="p-5 bg-secondary/20 rounded-lg border border-border/30 space-y-4">
        ...
        <div className="flex justify-between items-center pt-2 border-t border-border/30">
          <div className="flex items-center gap-2">
            <p className="text-sm text-foreground">Delivery</p>
            <span className="px-2 py-0.5 bg-primary/10 border border-primary/20 rounded text-xs font-semibold text-primary">GRATIS</span>
          </div>
          <p className="text-sm font-semibold text-muted-foreground line-through">Gs. 30.000</p>
        </div>

        {/* Priority Shipping Upsell */}
        <div
          onClick={() => setIsPriorityShipping(!isPriorityShipping)}
          className={`relative p-4 rounded-xl border cursor-pointer transition-all duration-300 group ${isPriorityShipping ? 'bg-primary/5 border-primary/40' : 'bg-secondary/30 border-border/40 hover:bg-secondary/50 hover:border-border/60'}`}
        >
          <div className="flex items-start gap-3">
            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 flex-shrink-0 mt-0.5 ${isPriorityShipping ? 'bg-primary border-primary scale-110' : 'border-muted-foreground/40 group-hover:border-primary/50'}`}>
              {isPriorityShipping && <CheckIcon className="w-3.5 h-3.5 text-foreground" strokeWidth={3} />}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2 mb-1">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <p className={`text-sm font-bold ${isPriorityShipping ? 'text-primary' : 'text-foreground'}`}>Envío Prioritario VIP</p>
                  <RocketLaunchIcon className={`w-4 h-4 flex-shrink-0 ${isPriorityShipping ? 'text-primary' : 'text-muted-foreground/70'}`} />
                </div>
                <p className={`text-sm font-bold whitespace-nowrap ml-2 ${isPriorityShipping ? 'text-primary' : 'text-muted-foreground'}`}>+ Gs. 10.000</p>
              </div>
              <p className="text-xs text-muted-foreground leading-relaxed">Despacho inmediato en 24hs</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center gap-3 pt-3 border-t border-border/50">
          <span className="text-base md:text-lg font-bold text-foreground">Total a pagar</span>
          <span className="text-xl md:text-2xl font-bold text-primary whitespace-nowrap">{formatPrice(finalTotal, currency)}</span>
        </div>
      ...
      {/* Buttons */}
      <div className="space-y-3">
        <Button type="submit" variant="hero" size="xl" className="w-full h-14 text-sm md:text-base" disabled={isProcessing}>
          {isProcessing ? (
            <>
              <div className="inline-block w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin mr-2" />
              Procesando pedido...
